...
def test_term_actions(users, user, action, term, rf):
    ...
    if action == 'add':
        ...
        url = reverse(
            'admin:letsagree_term_{0}'.format(action),
            args=(term.id, ),
        )
...
@pytest.mark.parametrize('main_user,other_user,action', [
    ('staff', 'sup_er', 'delete'),
    ('staff', 'sup_er', 'change'),
    ('staff', 'sup_er', 'add'),
    ('sup_er', 'staff', 'delete'),
...
def test_notarypublic_actions(main_user, other_user, action, agreed_users, rf):
    ...
    if action == 'add':
        ...
        url = reverse(
            'admin:letsagree_notarypublic_{0}'.format(action),
            args=(agreement_id, ),
        )
...
def test_notarypublic_changelist(agreed_users, user,
