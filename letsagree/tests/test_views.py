...
def test_view_structure(queries, django_assert_num_queries, settings,
                        terms_agreed, request_url, agree_queries):
...
def test_view_post(queries, admin_client, settings):
    ...
    formset = forms.PendingAgreementFormSet(
        data=data,
    )
    ...
    request = factory.post(
        '{0}?next=/'.format(reverse('letsagree:pending')),
        data,
    )
    ...
    assert tuple(request.user.agreed_terms.values_list('term_key_id')) == tuple(term_ids)
